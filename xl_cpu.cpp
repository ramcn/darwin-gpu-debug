...
unsigned int NtChar2Int_b(char nt) {
    switch(nt) {
        case 'a':
        case 'A': return A_NT;
        case 'c':
...

{
	...
	for (int index=1; index < MAX_TILE_SIZE+1 ;index++){
      		for (int k = 1; k < MAX_TILE_SIZE + 1; k++) {
                ...
                for (int j=1; j < MAX_TILE_SIZE+1 ; j++){
                        ...
                        int ref_nt = (reverse) ? NtChar2Int_b(a[m-index]) : NtChar2Int_b(a[index-1]);
                        int query_nt = (reverse) ? NtChar2Int_b(b[n-j]) : NtChar2Int_b(b[j-1]);
                        int match;
                        ...
                        if (h_matrix_wr[j] >= *max_score_arg) {
                                *max_score_arg = h_matrix_wr[j];
                                *max_i_arg = index;
                                *max_j_arg = j;
                        }
                        if ((j == query_pos) && (index == ref_pos)) {
                               *pos_score_arg = h_matrix_wr[j];
                        }
